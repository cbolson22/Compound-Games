...
export default function LoopaBoard({
  ...
  const { edges, elapsed, solved, litDots, toggleEdge, reset, restoreEdges } = useLoopa(puzzle, {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          if (data.solution) restoreEdges(data.solution as string[])
          getUserStreak(user.id, 'loopa').then(setStreak)
        }
        setLoadingScore(false)
      })
  }, [user, puzzleId, restoreEdges])
  ...
  }, [edges, storageKey, loadingScore, existingScore, solved])
  ...
  return (
    <div className={styles.board}>
      ...
      <div className={styles.sub}>Draw one closed loop — numbers show how many sides it uses</div>
