...
type PointScoreRow = { user_id: string; score: number; solution: string[][] | null; profiles: { username: string; show_verba_board: boolean } | null };
...
async function getPointScores(game: string): Promise<PointScoreRow[]> {
  ...
  const { data } = await supabase
    .from("scores")
    .select("user_id, score, solution, profiles(username, show_verba_board)")
    .eq("puzzle_id", puzzleId)
